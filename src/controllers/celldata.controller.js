    ...
    try {
        ...
        const data = rawData.map((row, index) => {
            ...
            return {
                ...
                Azimuth: row.Azimuth !== undefined && row.Azimuth !== '' ? parseInt(row.Azimuth) : null,
                Earfcn: parseInt(row.Earfcn),
                Freq: row.Freq !== undefined && row.Freq !== '' ? parseInt(row.Freq) : null,
                eNBName: row.eNBName || null,
                UserLabel: row.UserLabel || null,
                Longitude: row.Longitude !== undefined && row.Longitude !== '' ? parseFloat(row.Longitude) : null,
                Latitude: row.Latitude !== undefined && row.Latitude !== '' ? parseFloat(row.Latitude) : null
            };
        });
